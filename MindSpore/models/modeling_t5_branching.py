...
from mindnlp.transformers.ms_utils import ALL_LAYERNORM_LAYERS, find_pruneable_heads_and_indices, prune_linear_layer
from mindnlp.transformers.models.t5.configuration_t5 import T5Config
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def construct(
        ...
        if decoder_num is not None:
            bs, seq_len, dim = hidden_states.shape
            hidden_states_cat = []
            attention_mask_cat = []

            for i in range(len(decoder_num)):
                ex_num = decoder_num[i]
                hidden_states_item = hidden_states[i, :, :]
                hidden_states_item = hidden_states_item.view(1, seq_len, dim).expand(ex_num, seq_len, dim)
                hidden_states_cat.append(hidden_states_item)

                attention_mask_item = attention_mask[i, :]
                attention_mask_item = attention_mask_item.view(1, seq_len).expand(ex_num, seq_len)
                attention_mask_cat.append(attention_mask_item)


            hidden_states = mindspore.ops.cat(hidden_states_cat, 0)
            attention_mask = mindspore.ops.cat(attention_mask_cat, 0)
        ...
        if labels is not None:
            def zlpr_loss(logits, labels, label_masks):
                logits = ops.clamp(logits, -80, 80)
                logits_pos = ops.exp(-logits)
                logits_pos = ops.sum(logits_pos * labels * label_masks, dim=0)
                loss_pos = ops.log(1 + logits_pos)
                logits_neg = ops.exp(logits)
                logits_neg = ops.sum(logits_neg * (1 - labels) * label_masks, dim=0)
                loss_neg = ops.log(1 + logits_neg)
                loss = loss_pos + loss_neg
                return loss

            label_masks = label_masks.unsqueeze(-1).expand_as(lm_logits)
            offset = 0
            loss = 0
            for num in decoder_num:
                lm_logits_item = lm_logits[offset: offset + num, :, :]
                labels_branching_item = labels_branching[offset: offset + num, :, :]
                label_masks_item = label_masks[offset: offset + num, :, :]
                offset += num
                loss += zlpr_loss(lm_logits_item.view(-1),
                                  labels_branching_item.view(-1),
                                  label_masks_item.reshape(-1))
            loss = loss / len(decoder_num)
