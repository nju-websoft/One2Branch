...
from mindnlp.transformers.modeling_outputs import CausalLMOutputWithPast, Seq2SeqLMOutput
...
from mindnlp.transformers.generation.beam_search import BeamScorer, BeamSearchScorer, ConstrainedBeamSearchScorer

from mindnlp.transformers.generation.beam_constraints import DisjunctiveConstraint, PhrasalConstraint
    ...
    def branching_decoding(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            input_ids = input_ids[next_indices]
            unfinished_sequences = unfinished_sequences[next_indices]
            model_inputs['encoder_outputs']['last_hidden_state'] = model_inputs['encoder_outputs']['last_hidden_state'][next_indices]
            model_inputs['attention_mask'] = model_inputs['attention_mask'][
                next_indices.astype(model_inputs['attention_mask'].dtype)]
            model_kwargs['attention_mask'] = model_kwargs['attention_mask'][
                next_indices.astype(model_kwargs['attention_mask'].dtype)]
            # outputs.decoder_hidden_states = outputs.decoder_hidden_states[next_indices]

            if score_list is None:
                score_list = next_tokens_scores.unsqueeze(1)
                score_mask_list = unfinished_sequences.unsqueeze(1)
            else:
                score_list = score_list[next_indices]
                score_list = ops.cat([score_list, next_tokens_scores.unsqueeze(1)], 1)

                score_mask_list = score_mask_list[next_indices]
                score_mask_list = ops.cat([score_mask_list, unfinished_sequences.unsqueeze(1)], 1)
            ...
            if unfinished_sequences.min() == 0:
                ...
                if input_ids_pool is None:
                    ...
                    pad = ops.zeros((input_ids_pool.shape[0], input_ids.shape[1] - input_ids_pool.shape[1]),
                                     dtype=mindspore.int64)


                    input_ids_pool = ops.cat([input_ids_pool, pad], 1)
                    input_ids_pool = ops.cat([input_ids_pool, input_ids[idx]], 0)
                    # pad = ops.zeros([scores_pool.shape[0], score_list.shape[1]
                    #                 - scores_pool.shape[1]])
                    pad = pad.astype(scores_pool.dtype)
                    scores_pool = ops.cat([scores_pool, pad], 1)
                    scores_pool = ops.cat([scores_pool, score_list[idx]], 0)

                idx = unfinished_sequences.bool()

                input_ids = input_ids[idx]
                score_list = score_list[idx]
                unfinished_sequences = unfinished_sequences[idx]
                model_inputs['encoder_outputs']['last_hidden_state'] = \
                    model_inputs['encoder_outputs']['last_hidden_state'][idx]
                model_inputs['attention_mask'] = model_inputs['attention_mask'][idx]
                model_kwargs['attention_mask'] = model_kwargs['attention_mask'][idx]

            # stop when each sentence is finished, or if we exceed the maximum length
            if input_ids.shape[0] == 0 or unfinished_sequences.max() == 0 or stopping_criteria(input_ids, scores):
                if not synced_gpus:
                    break
                else:
                    this_peer_finished = True
