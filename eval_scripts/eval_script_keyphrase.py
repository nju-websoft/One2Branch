import numpy as np
import argparse
import eval_scripts.kp_config as kp_config
from eval_scripts.kp_utils.string_helper import *
from collections import defaultdict
import os
import eval_scripts.pykp.utils.io as io
...
def update_f1_dict(opt, trg_token_2dlist_stemmed, pred_token_2dlist_stemmed, k_list, f1_dict, tag):
...
def update_score_dict(opt, trg_token_2dlist_stemmed, pred_token_2dlist_stemmed, k_list, score_dict, tag):
    num_targets = len(trg_token_2dlist_stemmed)
    num_predictions = len(pred_token_2dlist_stemmed)
...
def update_score_dict_with_name_variation_backup(opt, is_match_all, pred_indices, num_predictions, num_targets, k_list,
                                                 score_dict, tag):
...
def update_score_dict_with_name_variation(opt, trg_variation_token_3dlist, pred_token_2dlist, k_list, score_dict, tag):
    num_targets = len(trg_variation_token_3dlist)
    num_predictions = len(pred_token_2dlist)
    is_match = compute_var_match_result(trg_variation_token_3dlist, pred_token_2dlist)
    # Classification metrics
...
def keyphrase_evaluate(src_file, trg_file, pred_file):
    parser = argparse.ArgumentParser(
        # description='evaluate_prediction.py',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    kp_config.post_predict_opts(parser)
    ...
    for data_idx, (src_l, trg_l, pred_l) in enumerate(
            zip(src_file, trg_file, pred_file)):
        ...
        filtered_stemmed_pred_token_2dlist, num_duplicated_predictions = filter_prediction(opt, opt.disable_valid_filter,
        ...
        if opt.use_name_variations:
            ...
            if opt.tune_f1_v:
                ...
                f1_dict = update_f1_dict(opt, unique_stemmed_trg_variation_token_3dlist, filtered_stemmed_pred_token_2dlist,
                                         topk_dict['all'], f1_dict, 'all')
                # compute F1 score present
                f1_dict = update_f1_dict(opt, present_unique_stemmed_trg_variation_token_3dlist,
                                         present_filtered_stemmed_pred_token_2dlist,
                                         topk_dict['present'], f1_dict, 'present')
                # compute F1 score absent
                f1_dict = update_f1_dict(opt, absent_unique_stemmed_trg_variation_token_3dlist,
                ...
                score_dict = update_score_dict_with_name_variation(opt, unique_stemmed_trg_variation_token_3dlist,
                                                                   filtered_stemmed_pred_token_2dlist,
                                                                   topk_dict['all'], score_dict, 'all')
                # compute all the metrics and update the score_dict for present keyphrase
                score_dict = update_score_dict_with_name_variation(opt, present_unique_stemmed_trg_variation_token_3dlist,
                                                                   present_filtered_stemmed_pred_token_2dlist,
                                                                   topk_dict['present'], score_dict, 'present')
                # compute all the metrics and update the score_dict for present keyphrase
                score_dict = update_score_dict_with_name_variation(opt, absent_unique_stemmed_trg_variation_token_3dlist,
                                                                   absent_filtered_stemmed_pred_token_2dlist,
            ...
            if opt.tune_f1_v:
                ...
                # compute all the metrics and update the score_dict
                score_dict = update_score_dict(opt, unique_stemmed_trg_token_2dlist, filtered_stemmed_pred_token_2dlist,
                                               topk_dict['all'], score_dict, 'all')
                # compute all the metrics and update the score_dict for present keyphrase
                score_dict = update_score_dict(opt, present_unique_stemmed_trg_token_2dlist,
                                               present_filtered_stemmed_pred_token_2dlist,
                                               topk_dict['present'], score_dict, 'present')
                # compute all the metrics and update the score_dict for present keyphrase
                score_dict = update_score_dict(opt, absent_unique_stemmed_trg_token_2dlist,
                                               absent_filtered_stemmed_pred_token_2dlist,
                                               topk_dict['absent'], score_dict, 'absent')
        if opt.export_filtered_pred:
            final_pred_str_list = []
    ...
    result_dict = {}
    for field, result in zip(field_list, result_list):
        result_dict[field] = round(result , 4)
...
if __name__ == '__main__':
    ...
    src_file_path = './datasets/Inspec/test_src.txt'
    trg_file_path = './datasets/Inspec/test_trg.txt'
    # pred_file_path = './datasets/Inspec/predictions.txt'
    pred_file_path = './datasets/Inspec/predictions.txt'
    src_file, trg_file, pred_file = open(src_file_path), open(trg_file_path), open(pred_file_path)


    # logging = kp_config.init_logging(log_file=opt.exp_path + '/evaluate_prediction_result.log', stdout=True)
    # logging.info('Parameters:')
    # [logging.info('%s    :    %s' % (k, str(v))) for k, v in opt.__dict__.items()]

    result_dict = keyphrase_evaluate(src_file, trg_file, pred_file)
    import json


    macro_avg_f1_5 = result_dict['macro_avg_f1@5_all']
    macro_avg_f1_M = result_dict['macro_avg_f1@M_all']
    print(macro_avg_f1_5, macro_avg_f1_M)

    macro_avg_f1_5_present = result_dict['macro_avg_f1@5_present']
    macro_avg_f1_M_present = result_dict['macro_avg_f1@M_present']
    print(macro_avg_f1_5_present, macro_avg_f1_M_present)

    macro_avg_f1_5_absent = result_dict['macro_avg_f1@5_absent']
    macro_avg_f1_M_absent = result_dict['macro_avg_f1@M_absent']
    print(macro_avg_f1_5_absent, macro_avg_f1_M_absent)
