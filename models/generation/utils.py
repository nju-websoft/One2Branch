...
class GenerationMixin:
    ...
    def generate(
        ...
        branching_decoding: Optional[bool] = False,
        min_beam_num: Optional[int] = None,
        max_beam_num: Optional[int] = None,
        **kwargs,
        ...
        if branching_decoding:
            return self.branching_decoding(
                input_ids,
                logits_processor=logits_processor,
                ...
                min_beam_num=min_beam_num,
                max_beam_num=max_beam_num,
                **model_kwargs,
            )
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # print('next_tokens1:', next_tokens.shape)
            # finished sentences should have their next token be a padding token
            if eos_token_id is not None:
                if pad_token_id is None:
                    raise ValueError("If `eos_token_id` is defined, make sure that `pad_token_id` is defined.")
    ...
    def branching_decoding(
        ...
        step = 0
        score_list = None
        score_mask_list = None
        decoder_hidden_states = None
        input_ids_pool, scores_pool = None, None
        while True:
            step += 1
            if synced_gpus:
                # Under synced_gpus the `forward` call must continue until all gpus complete their sequence.
            ...
            # argmax
            cur_beam_num, vocab_size = next_tokens_scores.size()
            # if score_list is not None:
            #     cur_seq_len = score_list.size(1)
            #
            #     score_list_ex = score_list.unsqueeze(dim=2).expand(cur_beam_num, cur_seq_len, vocab_size)
            #     next_tokens_scores_ex = torch.cat([score_list_ex, next_tokens_scores.unsqueeze(dim=1)], dim=1)
            #
            #     score_mask_list_ex = score_mask_list.unsqueeze(dim=2).expand(cur_beam_num, cur_seq_len, vocab_size)
            #     unfinished_sequences_ex = unfinished_sequences.unsqueeze(dim=1).expand(cur_beam_num, vocab_size)
            #     score_mask_list_ex = torch.cat([score_mask_list_ex, unfinished_sequences_ex.unsqueeze(dim=1)], dim=1)
            #
            #     next_tokens_scores_tmp = torch.sum(next_tokens_scores_ex * score_mask_list_ex, dim=1) / torch.sum(
            #         score_mask_list_ex, dim=1)
            #     next_tokens_scores_tmp = next_tokens_scores_tmp.reshape(-1)
            # else:
            #     next_tokens_scores_tmp = next_tokens_scores.reshape(-1)
            # next_tokens_scores_tmp = next_tokens_scores_tmp.reshape(-1)

            next_tokens_scores = next_tokens_scores.reshape(-1)
            next_tokens_scores, next_tokens = torch.topk(
                next_tokens_scores, max_beam_num, dim=0, largest=True, sorted=True
            )

            gt0_idx = next_tokens_scores > 0
            gt0_count = torch.sum(gt0_idx.long(), dim=0)
            gt0_count = gt0_count.item()

            pool_num = 0
            if scores_pool is not None:
                pool_num = scores_pool.size(0)
            if gt0_count < min_beam_num - pool_num:
                gt0_count = min_beam_num
            if gt0_count <= 0:
                break

            next_tokens = next_tokens[:gt0_count]
            next_tokens_scores = next_tokens_scores[:gt0_count]
            next_indices = torch_int_div(next_tokens, vocab_size)
            next_tokens = next_tokens % vocab_size


            input_ids = input_ids[next_indices.to(input_ids.device)]
            unfinished_sequences = unfinished_sequences[next_indices]
            model_inputs['encoder_outputs']['last_hidden_state'] = model_inputs['encoder_outputs']['last_hidden_state'][
                next_indices.to(model_inputs['encoder_outputs']['last_hidden_state'].device)]
            model_inputs['attention_mask'] = model_inputs['attention_mask'][next_indices.to(model_inputs['attention_mask'].device)]
            model_kwargs['attention_mask'] = model_kwargs['attention_mask'][next_indices.to(model_kwargs['attention_mask'].device)]
            # outputs.decoder_hidden_states = outputs.decoder_hidden_states[next_indices]

            if score_list is None:
                score_list = next_tokens_scores.unsqueeze(dim=1)
                score_mask_list = unfinished_sequences.unsqueeze(dim=1)
            else:
                score_list = score_list[next_indices]
                score_list = torch.cat([score_list, next_tokens_scores.unsqueeze(dim=1)], dim=1)

                score_mask_list = score_mask_list[next_indices]
                score_mask_list = torch.cat([score_mask_list, unfinished_sequences.unsqueeze(dim=1)], dim=1)
            ...
            if unfinished_sequences.min() == 0:
                idx = 1 - unfinished_sequences
                idx = idx.bool()

                if input_ids_pool is None:
                    input_ids_pool = input_ids[idx]
                    scores_pool = score_list[idx]

                else:
                    input_ids_pool = torch.cat(
                        [input_ids_pool,
                         torch.zeros([input_ids_pool.size(0), input_ids.size(1) - input_ids_pool.size(1)],
                                     dtype=torch.long, device=input_ids_pool.device)], dim=1)
                    input_ids_pool = torch.cat([input_ids_pool, input_ids[idx]], dim=0)

                    scores_pool = torch.cat(
                        [scores_pool,
                         torch.zeros([scores_pool.size(0), score_list.size(1) - scores_pool.size(1)],
                                     dtype=torch.long, device=scores_pool.device)], dim=1)
                    scores_pool = torch.cat([scores_pool, score_list[idx]], dim=0)

                idx = unfinished_sequences.bool()
                input_ids = input_ids[idx.to(input_ids.device)]
                score_list = score_list[idx.to(score_list.device)]
                unfinished_sequences = unfinished_sequences[idx.to(unfinished_sequences.device)]
                model_inputs['encoder_outputs']['last_hidden_state'] = \
                    model_inputs['encoder_outputs']['last_hidden_state'][
                        idx.to(model_inputs['encoder_outputs']['last_hidden_state'].device)]
                model_inputs['attention_mask'] = model_inputs['attention_mask'][
                    idx.to(model_inputs['attention_mask'].device)]
                model_kwargs['attention_mask'] = model_kwargs['attention_mask'][
                    idx.to(model_kwargs['attention_mask'].device)]

            # stop when each sentence is finished, or if we exceed the maximum length
            if input_ids.size(0) == 0 or unfinished_sequences.max() == 0 or stopping_criteria(input_ids, scores):
                if not synced_gpus:
                    break
                else:
                    this_peer_finished = True
        ...
        if return_dict_in_generate:
            if input_ids_pool is None:
                input_ids_pool = input_ids
                scores_pool = score_list
            score_list_new = []
            for score_list_item, input_ids_item in zip(scores_pool.tolist(), input_ids_pool[:, 1:].tolist()):
                score_list_new_item = []
                for score, input_id in zip(score_list_item, input_ids_item):
                    score_list_new_item.append(score)
                    if input_id == 1:
                        break
                score_list_new.append(score_list_new_item)

            if self.config.is_encoder_decoder:

                return GreedySearchEncoderDecoderOutput(
                    sequences=input_ids_pool,
                    scores=scores,
                    score_list=score_list_new,
                    encoder_attentions=encoder_attentions,
                    encoder_hidden_states=encoder_hidden_states,
                    decoder_attentions=decoder_attentions,
                    cross_attentions=cross_attentions,
